cc.Class({
    ...
    onLoad: function () {
        ...
        for(var y = 0;y<15;y++){
            for(var x = 0;x < 15;x++){
                ...
                newNode.setPosition(cc.Vec2(x*40+20,y*40+20));//根据棋盘和棋子大小计算使每个棋子节点位于指定位置
                newNode.tag = y*15+x;//根据每个节点的tag就可以算出其二维坐标
                newNode.TT = y*15+x;//根据每个节点的tag就可以算出其二维坐标
                newNode.on(cc.Node.EventType.TOUCH_END,function(event){
                    self.touchChess = this;
                    console.log("--------------------0",self.gameState,this.getComponent(cc.Sprite).spriteFrame)
    ...
    judgeOver:function(){
        var x0 = this.touchChess.TT % 15;
        var y0 = parseInt(this.touchChess.TT / 15);
        //判断横向
        var fiveCount = 0;
        console.log("-----------------------------this.touchChess.name:",this.touchChess.TT)
        console.log("-----------------------------x0:",x0)
        console.log("-----------------------------y0:",y0)
        console.log("-----------------------------:",this.chessList[0].getComponent(cc.Sprite).spriteFrame)
        for(var x = 0;x < 15;x++){
